...
from src.core.models import Creator, Campaign, Application, FAQ, Payment
from src.core.config import SAMPLE_DATA_DIR
...
def search_creators(creators: list[Creator], query: str) -> list[Creator]:
    ...
    return [
        ...
        or q in c.primary_niche.lower()
        or q in c.primary_language.lower()
...
def search_campaigns(campaigns: list[Campaign], query: str) -> list[Campaign]:
    ...
    return [
        c
        for c in campaigns
        if q in c.title.lower() or q in c.brand.lower() or q in c.description.lower()
    ]
