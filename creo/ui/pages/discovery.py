...
try:
    ...
    with st.container(border=True):
        st.subheader("Creator attributes")
        c1, c2, c3 = st.columns(3)
        with c1:
            niche = st.selectbox("Niche", ["All"] + sorted(get_all_niches()))
            language = st.selectbox("Language", ["All"] + sorted(get_all_languages()))
        with c2:
            region = st.text_input("Region", placeholder="e.g. IN-MH, US, UK, BR")
            platform = st.selectbox("Platform", ["instagram", "facebook", "threads"])
        with c3:
            min_f = st.number_input("Min followers", min_value=0, value=0, step=1000)
            max_f = st.number_input("Max followers (0 = no limit)", min_value=0, value=0, step=1000)
