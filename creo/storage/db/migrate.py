"""One-shot migration: reads all JSON files and writes to SQLite."""
...
from creo.storage.db.orm_models import (
    DbCreator, DbCampaign, DbPayment, DbAssignment, DbFollowUpNote,
)
...
def migrate():
    ...
    for d in notes_data:
        n = FollowUpNote(**d)
        session.add(DbFollowUpNote(id=n.id, campaign_id=n.campaign_id, note=n.note, created_at=n.created_at))
